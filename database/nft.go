...
func (db *Db) SaveNFTEvent(eventType string, nftAddress, validator, newOwner, owner string, amount sdk.DecCoins) error {
	query := `
		INSERT INTO nft_events(event_type, nft_address, owner, new_owner, validator, amount) 
		VALUES ($1, $2, $3, $4, $5, $6)
	`
	_, err := db.SQL.Exec(query, eventType, nftAddress, owner, newOwner, validator, amount)
	if err != nil {
...
func (db *Db) SaveNFT(address, owner string, lockedAmount, availableAmount sdk.DecCoins, delegations []types2.NFTDelegation) error {
	query := `
